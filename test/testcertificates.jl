certificates = getallcertificates!(testclient)

@testset "List all Certificates" begin
    @test length(certificates) == 2
    @test certificates[2].id == "ba9b9c18-6c59-46c2-99df-70da170a42ba"
    @test certificates[2].name == "web-cert-02"
    @test certificates[2].notafter == DateTime("2018-06-07T17:44:12")
    @test certificates[2].sha1fingerprint == "479c82b5c63cb6d3e6fac4624d58a33b267e166c"
    @test certificates[2].created_at == DateTime("2018-03-09T18:44:11")
    @test certificates[2].dnsnames == ["www.example.com", "example.com"]
    @test certificates[2].state == "verified"
    @test certificates[2].certificatetype == "lets_encrypt"
end;

certificate = getcertificate!(testclient, "892071a0-bb95-49bc-8021-3afd67a210bf")

@testset "Retrieve an existing Certificate" begin
    @test certificate.id == "892071a0-bb95-49bc-8021-3afd67a210bf"
    @test certificate.name == "web-cert-01"
    @test certificate.notafter == DateTime("2017-02-22T00:23:00")
    @test certificate.sha1fingerprint == "dfcc9f57d86bf58e321c2c6c31c7a971be244ac7"
    @test certificate.created_at == DateTime("2017-02-08T16:02:37")
    @test length(certificate.dnsnames) == 1 && certificate.dnsnames[1] == ""
    @test certificate.state == "verified"
    @test certificate.certificatetype == "custom"
end;

certificate = createcertificate!(testclient; name="web-cert-01",
                                             private_key="-----BEGIN PRIVATE KEY-----\\neTuK2xNR0PIM8OI7pRpgyj1I\n-----END PRIVATE KEY-----",
                                             leaf_certificate="-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
                                             certificate_chain="-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----")

@testset "Create a new Certificate" begin
    @test certificate.id == "892071a0-bb95-49bc-8021-3afd67a210bf"
    @test certificate.name == "web-cert-01"
    @test certificate.notafter == DateTime("2017-02-22T00:23:00")
    @test certificate.sha1fingerprint == "dfcc9f57d86bf58e321c2c6c31c7a971be244ac7"
    @test certificate.created_at == DateTime("2017-02-08T16:02:37")
    @test length(certificate.dnsnames) == 1 && certificate.dnsnames[1] == ""
    @test certificate.state == "verified"
    @test certificate.certificatetype == "custom"
end;

@testset "Delete a Certificate" begin
    @test deletecertificate!(testclient, "892071a0-bb95-49bc-8021-3afd67a210bf")
    @test deletecertificate!(testclient, certificate)
end;
